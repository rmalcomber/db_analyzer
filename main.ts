...
function getRustType(pgType: string, isNullable: boolean): string {
  ...
  if (pgType.endsWith("[]")) {
    ...
    const baseRustType = pgToRustTypeMap[baseType] || pgToRustTypeMap["default"];
  ...
  try {
    ...
    for (const table of tablesResult.rows) {
      ...
      for (const column of columnsResult.rows) {
        ...
        rustOutput += `    /// ${originalColumnName} - ${column.data_type}${isNullable ? ", nullable" : ""}${column.column_default ? `, default: ${column.column_default}` : ""}\n`;
        
        // Add Serde attribute for mapping to database column name if different from field name
    ...
    try {
      ...
      console.warn("Could not parse database name from connection string, using default name");
    }
